...
@Component({
  ...
  template: `
    ...
    <section class="section-padding bg-[#eef4ff]" [attr.aria-label]="content.LATEST_NOTICES.AREA_LABEL">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row gap-4 mb-8">
          <div class="flex-1 relative">
            ...
            <input type="text" [placeholder]="content.LATEST_NOTICES.SEARCHPLACEHOLDER" [ngModel]="searchQuery()" (ngModelChange)="searchQuery.set($event)"
              class="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm" [attr.aria-label]="content.LATEST_NOTICES.SEARCHPLACEHOLDER"
            />
          </div>
          <div class="flex flex-wrap gap-2">
            @for (cat of content.CATEGORIES; track $index; let i = $index) {
              <button (click)="selectedCategory.set(cat)" class="px-4 py-2.5 rounded-xl text-sm font-medium transition-all"
        ...
        <div class="space-y-4">
          @for (notice of filteredNotices(); track $index; let i = $index) {
              ...
              <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                <div class="sm:w-20 sm:text-center shrink-0">
                  ...
                  <div class="text-xs text-muted uppercase">
                    {{ getMonth(notice.date) }} {{ getYear(notice.date) }}
                  </div>
                </div>
                <div class="flex-1">
                  ...
                  <h3 class="text-lg font-bold font-heading text-dark mb-1">
                    {{ notice.title }}
                  </h3>
...
export class NoticesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly filteredNotices = computed(() => {
    let notices = this.content.LATESTNOTICE_LIST;
  ...
  getMonth(d: string): string {
    return new Date(d).toLocaleString('en', { month: 'short' });
  }
  getYear(d: string): string {
